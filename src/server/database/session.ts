...
import {UserId, withClient} from "./core";
...
export async function create(userId: UserId): Promise<string> {
  ...
  await withClient(async (client) => {
    await client.query(
      `INSERT INTO sessions ("user", key, expire) VALUES ($1, $2, NOW() + INTERVAL '14 days')`,
      [userId.name, key],
    );
  });
...
export async function invalidateUser(userId: UserId): Promise<void> {
  await withClient(async (client) => {
    await client.query(`DELETE FROM sessions WHERE "user" = $1`, [userId.name]);
  });
}
...
export async function authenticate(sessionKey: string): Promise<UserId | null> {
  const result = await withClient(async (client) => {
    return await client.query(`SELECT "user" FROM sessions WHERE key = $1 AND expire > NOW()`, [sessionKey]);
  });
